...
function extractLevel(html: string): string {
  // Use minimum non-zero severity: the lowest non-zero level found is the general country advisory.
  // Zero-severity matches ("inga särskilda") are ignored when any higher advisory is present,
  // because they appear in regional "no restriction" sub-sections on compound pages.
  ...
  if (matches.length === 0) return "Inga särskilda restriktioner";
...
function extractSummary(html: string): string {
  const clean = html
    ...
    .replace(/<header[\s\S]*?<\/header>/gi, "")
    .replace(/<footer[\s\S]*?<\/footer>/gi, "");
  const match = clean.match(/<div[^>]*class="[^"]*article-body[^"]*"[^>]*>([\s\S]{0,15000})/i)
    ?? clean.match(/<article[^>]*>([\s\S]{0,15000})/i)
