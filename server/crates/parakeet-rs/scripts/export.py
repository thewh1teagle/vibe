...
def convert(model_spec: str, out_path: Path, repo_id: str | None = None) -> None:
    ...
    # Prefer the explicit/source HF identity. An arbitrary explicit output
    # path need not have the variant slug as its parent directory.
    identity = repo_id or (model_spec if "/" in model_spec and not Path(model_spec).exists() else None)
    slug = slug_from_repo_id(identity) if identity else out_path.parent.name
