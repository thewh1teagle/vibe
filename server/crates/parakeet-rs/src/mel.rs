    ...
    pub fn compute(&self, pcm: &[f32]) -> Result<MelFeatures> {
        ...
        if pcm.len() < pad + 1 {
            return Err(Error::Load(
                "audio is too short for Parakeet's reflect-padded frontend".into(),
            ));
        }
        // NeMo reports floor(n_samples / hop) valid frames even though
        // centered padding makes one additional FFT window available.
        let num_frames = pcm.len() / cfg.hop_length;
        ...
        // NeMo FilterbankFeatures(normalize="per_feature"): sample
        // variance over time (Bessel correction), then epsilon is added
        // to std outside sqrt.
        let denominator = num_frames.saturating_sub(1).max(1) as f64;
        for m in 0..cfg.num_mels {
            let row = &mut output[m * num_frames..(m + 1) * num_frames];
            let mean = row.iter().map(|&value| value as f64).sum::<f64>() / num_frames.max(1) as f64;
            let variance = row
                .iter()
                .map(|&value| {
                ...
                .sum::<f64>()
                / denominator;
            let scale = 1.0 / (variance.sqrt() as f32 + 1.0e-5);
            for value in row {
                *value = (*value - mean as f32) * scale;
            }
        }
        Ok(MelFeatures {
            data: output,
