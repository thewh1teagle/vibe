						...
						<div className={cn('space-y-0.5', hasSummary && 'border-t border-border pt-2')}>
							...
							{hasSummary && (
								<button
									...
									onClick={() => {
										setOpen(false)
										if (job?.summary) void clipboard.writeText(job.summary)
									}}
									className="flex w-full cursor-pointer items-center gap-2.5 rounded-lg px-2 py-2 text-start text-sm text-foreground transition-colors duration-150 hover:bg-muted/70">
...
export default function TranscriptToolbar({
	...
	async function onCopy() {
		if (tab === 'summary' && job?.summary) await clipboard.writeText(job.summary)
		else await copy()
		setCopied(true)
		if (resetRef.current) window.clearTimeout(resetRef.current)
