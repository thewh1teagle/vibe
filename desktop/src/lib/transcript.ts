...
export function formatTimestamp(
  ...
  const hoursMarker =
    alwaysIncludeHours || hours !== 0 ? `${hours}:` : "";

  return `${hoursMarker}${String(minutes).padStart(
    2,
    "0"
  )}:${String(formattedSeconds).padStart(
    2,
    "0"
  )}${decimalMarker}${String(milliseconds).padStart(
    3,
    "0"
  )}`;
}
...
export function asSrt(transcript: Transcript) {
  const wordUtterances =
    transcript.utterances || transcript.word_utterances;
  return wordUtterances.reduce(
    (transcript, fragment, i) => {
      return (
        transcript +
        `\n${i + 1}\n` +
        `${formatTimestamp(
...
export function asText(transcript: Transcript) {
  return transcript.utterances.reduce(
    (transcript, fragment) => {
