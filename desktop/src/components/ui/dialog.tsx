	...
	({ className, children, ...props }, ref) => (
		<DialogPortal>
			...
			<DialogPrimitive.Content
				ref={ref}
				className={cn(
					"fixed start-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] rtl:-translate-x-[-50%] translate-y-[-50%] gap-4 border border-border/70 bg-card/98 p-6 text-foreground shadow-xl duration-200 data-[state=open]:animate-in-dialog data-[state=closed]:animate-out-dialog sm:rounded-xl max-h-[85vh] overflow-y-auto",
					className,
				)}
				{...props}>
				{children}
				<DialogPrimitive.Close className="absolute end-4 top-4 rounded-md border border-transparent p-1.5 text-muted-foreground/85 transition-colors hover:border-border/70 hover:bg-accent/55 hover:text-foreground focus:outline-none focus:ring-2 focus:ring-ring/80 disabled:pointer-events-none data-[state=open]:bg-accent/55">
					<X className="h-4 w-4" />
