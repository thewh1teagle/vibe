...
export default function DropModal() {
    ...
    async function handleDrops() {
        listeners.current.push(await event.listen("tauri://drop-over", (event: any) => {
            const newPosition = { x: event.payload.position.x, y: event.payload.position.y as any }
            setPosition(newPosition)
        }))
        listeners.current.push(await event.listen('tauri://drag', async (event) => {
        ...
        listeners.current.push(await event.listen('tauri://drag-cancelled', (_) => {
            setOpen(false)
        }))

        listeners.current.push(await event.listen('tauri://drop', () => {
            setOpen(false)
        }))
    }
    ...
    return (
        <div>
            {open && <Document path={path} position={position} />}
            <div className={cx('modal backdrop-blur-sm bg-base-100', open && 'modal-open')}>
