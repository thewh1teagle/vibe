...
<div class="flex gap-3 flex-col lg:flex-row">
	...
	{#if isMobile}
		<button on:mousedown={ctaClick} class="v-btn v-btn-primary">
			{t('download')}
		</button>
		<!-- macOS cta -->
	{:else if asset?.platform.toLowerCase() === 'macos'}
		<button on:mousedown={ctaClick} class="v-btn v-btn-primary hidden lg:flex">
			<MacIcon />
	...
	{:else if asset?.platform.toLowerCase() === 'windows'}
		<a
			...
			href={asset?.url}
			class="v-btn v-btn-primary hidden md:flex">
			<WindowsIcon />
	...
	{:else if asset?.platform.toLowerCase() === 'linux'}
		<button
			...
			<dialog id="linux_download_model" class="v-dialog w-11/12 max-w-5xl p-6">
				<div>
				...
				<form method="dialog" class="mt-6 flex justify-end">
					<button class="v-btn v-btn-ghost v-btn-sm">Close</button>
				</form>
			</dialog>
	{/if}

	<a class="v-btn v-btn-outline" href="https://github.com/thewh1teagle/vibe" target="_blank">
...
{#if asset?.platform.toLocaleLowerCase() == 'macos' && ctaClicked}
	<div class="flex gap-2 mt-3">
		<a
				...
				class="v-btn v-btn-sm v-btn-outline"
...
<div class="flex gap-3 mt-4">
	<button class="v-btn v-btn-ghost v-btn-icon" on:mousedown={() => changePlatform('macos')}><MacIcon /></button>

	<button
		class="v-btn v-btn-ghost v-btn-icon"
		aria-label="Windows"
		on:click={() => {
...
<dialog class="v-dialog w-[92vw] max-w-md p-6" open={mobileModalOpen} on:close={() => (mobileModalOpen = false)}>
	<div>
		...
		<div class="flex justify-center">
			<button on:click={() => navigator.clipboard.writeText(currentURL)} class="v-btn v-btn-primary">{t('copy-download-link')}</button>
		</div>

		<div class="mt-4 flex justify-end">
			<form method="dialog">
				<button on:click={() => (mobileModalOpen = false)} class="v-btn v-btn-ghost">{t('cancel')}</button>
			</form>
		</div>
	</div>
</dialog>
